...
选菜菜单使用程序生成的同心双作物环、分隔线、填色和命中多边形，十二种田地作物每环六种同时显示；菜架只有丝瓜时自动缩为单环。当前采用用户确认的方案 A 细框：外框的数学环宽由外半径的 13.5% 收到 7%，中心圆框同比收细，如意节点缩小到 48–60 px，分隔线和环间缝也随之变细。外圆框以精确四分之一圆环旋转复用，中心框使用完整数学圆环；四个接缝复用一张透明四合如意节点。图片工具只在固定透明模板内生成浅赭工笔木料，最终运行图重新套用数学遮罩，不把文字、图标、悬停态或点击区域烘焙进图片。按钮、弹窗和主要菜单面板使用同系列 4.2% 细边宣纸木框九宫格：固定四角与框宽，只拉伸边中段和中心，因此可独立改变宽高。旧厚框、写实平面木纹和旧节点保留为制作历史，不再用于当前菜单。完整原图、模板、实际提示词、哈希和处理步骤见 [materials-generation.json](RadialMenu/materials-generation.json) 及 [generation.json](RadialMenu/generation.json)；图片工具未返回费用。定向入口为 `tests/field_menu_scene_test.gd -- --visual`，覆盖十二种菜双环、标准圆形素材、九宫格主题、边缘收进、菜架单环、旋转复用节点、取消及实际点击。
