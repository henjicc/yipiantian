...
- 定向检查入口 `tests/animal_behavior_test.gd`：真实主场景、隔离存档，模拟360秒，检查7只活动边界、相互间距、单步位移、活动跨度、状态出现和反复卡住；`-- --visual`附近景连续相位截图。最终当前检查中鸭鹅累计54–73米、两鸡58/60米，路径恢复0–5次，鸡最大跨度约9.7×6米。模拟每步约0.4–0.8ms，含检查开销，不是整机帧耗时或GPU结论。
- 证据 `.local/animal-behavior/{check,visual,lifecycle}.log` 和 `.local/verification/animal-behavior/`。步行近景已观察到两脚交替；啄食／探水接触幅度、昼夜动态以及阶段独立可运行包仍待收尾，第07项目标未完成。图形检查曾出现退出资源残留提示，headless生命周期复查无残留，已延后退出两帧，下一次图形收尾继续核验，不视为已解决。
