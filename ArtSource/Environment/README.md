# 正式院落资产（3.2）

本目录交付能独立导入的可编辑环境源、真实生成来源、同源高低档与可复现的精确模块。玩法、保存、解锁、声音和动态水不属于本目录职责。

## 来源与分工

- 2026-09-21 续作 [十张独立远山扩景](Backdrop/README.md)：expanded-v5 新增十次图片生成，按不同方位与距离交错组合，覆盖受限左右旋转；补六处既有香蒲／荷叶组合，前景专用网格固定细节并平滑淡出。原图、透明通道、完整提示词及实景验证均保留，没有新增三维生成费用。

- 2026-09-21 [方向天空与真实山岸](Backdrop/README.md)：三张新生成原生透明山图用于无限远天空，四段低矮三维岸脚承接既有岛群与同一湖面；较高的光滑山包根据用户反馈替换为 individual-v4 的三个独立透明工笔小山／林岸；取代倾斜的 2.5D 山卡舞台。图片生成、真实几何与共享雾各自职责、提示词、实景验证入口见该记录，本轮没有新增 Tripo 调用。

- 2026-09-18 [远景岛群、水生植物与景深](Archipelago/README.md)：五种新岛及芦苇、香蒲、菱叶共八张独立参考／八个模型，480积分。十座邻居岛统一世界尺度，以三维位置产生透视；沿真实岸线分簇水草、补岛间浮叶，修复水面过早渐隐与远景虚化过渡过长。

- 2026-09-18 [水乡邻居岛](Islets/README.md)：三张独立参考生成三种完整农家岛（180积分），Blender整理高低档，在左右及远层布置五处真实三维院落。[圆润岸坡](Banks/README.md)重建主岛与桥头右岸；[透明远景第二版](Backdrop/README.md)使用三张不同山岸轮廓，各只出现一次。旧模型与背景属于历史源，现役重建以这三个入口为准。

- 2026-09-17 [松土与种植反馈](Soil/README.md)：两张内置图片工具输出分别用于土畦参考和正式土壤纹理；Godot 连续网格、少量合并土块、贴地选中提示、分阶段培土及短时土屑。调研、实际工具分工、原始提示词和补拍入口见该记录，本轮没有新增 Tripo 生成。

- 2026-09-17 [五种新岸石](Rocks/README.md)：五张独立图片工具参考 → 五次 H3.1 Ultra + v3.5 HD 图生模型，共300积分。现役 `stone_0..4` 来自 `Rocks/20260917/prepare.py`，复用于岸边、石板路和田块压边；下述 `build_modules.py` 的河石为历史版本，不能直接覆盖现役资源。
- 2026-09-17 高级能力小样：正式 [荷花](Lotus/README.md) 改为 P2 四边面 + v3.5 HD（120 积分）；正式 [民居](House/README.md) 改为 H3.1 Ultra + v3.5 HD（60 积分）。两版未采用的房屋重贴图各 20 积分，本轮总计 220 积分。新源各自放在版本子目录，旧源不覆盖；当前重建须使用该版本的 `prepare.py`。下面九件标准 H3.1 是历史生产记录，不再代表这两件资产的现役模型。两件新资产使用人工高低档，禁用其导入自动 LOD，其他资产策略不变。
- 内置 image_gen：以固定青菜与全景图为风格参考，生成船、庭院树、竹、野花、荷叶、藤架和三件装饰的单体图。野花去除误带的白粗茎、荷叶去除光晕，各仅定向修正一次；所有生成/修正均使用图像工具。
- Tripo CLI 0.4.0：九个原始 image_to_model 任务，全为真实 `v3.1-20260211`，texture=true、PBR=false、smart_low_poly=true、standard 纹理、original_image 对齐、image_autofix=false。各 **40 积分，共 360 积分**；没有为高低档重复付费。逐项真实 ID、请求参数、成功时间与费用见各目录 `tripo-original/task.json`（已移除签名下载URL）。
- Blender 5.2.2 LTS：`prepare_assets.py` 统一米制、接地/顶挂原点、同源低档、清理/法线、GLB重导入；`build_modules.py` 制作自然岛岸、五种河石、石桥拱券/栏杆、竹栏、入口棚、側屋瓦顶/山墙/窗门、廊台、田框；`House/repair_ridge.py` 只闭合原民居屋脊两端局部缺口，闭合塞用匹配的深灰瓦色，中段原纹理保留，原始 Tripo 模型不动。
- Godot 4.7.2：独立对象组装、地表/石材温和色层、自然间距、低档独立可控；独立实机验证环境 `Game/scenes/environment/environment_sample.tscn`。所有镜头截图均为真引擎输出，不是概念图覆写。
- 远景：`Backdrop/river-distance.png` 为单独生成的山水远岸画，不包含本场院落或交互物件；映射完整环形曲面，水面远处渐隐承接。允许平面远景，近景全为三维对象。

每项图像工具未单独返回模型版本、种子或价格，记录为未返回。用户要求暂不执行商用身份/授权调查，不把此项写成已通过。此处为开发侧视觉检查，不冒称用户已审美确认。

## 工程接口

`res://scenes/environment/courtyard.tscn` 可独立实例化；不改主场景、不加载农场状态。它仅依赖纯定义 `farm/decoration_catalog.gd` 的 ID/类型/可旋转角数。

- `get_decoration_slots()` 返回8槽 `id,type,transform,allowed_turns,radius`；`get_slot_marker(id)` 返回实际 Marker3D。坐标唯一真相在本场景，3.3存槽ID及quarter_turn，不另存坐标。
- 地面 `ground_01..04` 半径 .45m，允许0/90/180/270°；悬挂 `hanging_01..04` 固定0°，顶环归零、灯体向下 .65m，实体杆/绳接点。
- `get_decoration_scene(item_id,low_detail)` 返回 PackedScene；陶罐玩法ID `pot` 映射资源 `jar`；另有 `flowerpot,lantern`。
- `get_water_surface()` 返回 `WaterSurface`，2000×2000m XZ水平平面、法线上+Y、y=-.25；昼夜控制器接管 `material_override`，与真实山岸共用水位，远处渐入地平线雾色。
- `get_backdrop_material()` 返回方向天空 ShaderMaterial，昼夜控制器将其接入 Environment 的 Sky，并同步天空、远山与水面颜色；不使用全时白天的固定未着色背景。
- `get_asset_keys()` / `set_asset_detail(key,low_detail)` / `set_low_detail_enabled(bool)` 可独立控制复杂对象，高低来自同一网格与颜色贴图。简单测量模块共用单档，保留Godot导入自动LOD。
- 六田仍为两排三列、中心 `(-3.3+col*3.25,.2,row*2.8)`；96格土面、渐低土畦及交互由 `farm_layout.gd` 管理，环境不再叠加木田框。`ground_cover.gd` 生成一张合并的短草网格及仅投射到岛顶的根部土色贴花，不持有作物状态。
- 西侧旧入口棚与葫芦架不再实例化；`climbing_trellis.glb` 为4.64米四段竹架，局部长轴X旋转90°后沿西侧栏杆布置。连续土床仅预留后续攀爬作物；`hanging_03` 保留稳定ID，坐标由场景统一移到朝院内的挂臂。

2026-09-17质感修订：五种河石采用四层不规则环、三段倒角和平滑／加权法线，492／592／510／564／526三角；竹架4140三角、两材质。修改由 `build_modules.py --only stone_0 stone_1 stone_2 stone_3 stone_4 climbing_trellis` 重建，GLB均经重新导入核验。另做焊接导出法线接缝后的拓扑检查，六件资源均无边界边、非流形边、孤点和退化面；记录在本地 `.local/verification/surface-review/topology.json`。原始Tripo植物仍保留，当前未再生成或付费。

## 验证与边界

各 `asset-audit.json` 记录真实高低三角、尺寸、材质、纹理、非流形边、孤点及重导入结果。非流形边来自叶/窗/瓦等生成细节，不承诺打印/室内/物理船舶用途。复杂资产轮廓及开放结构另用实机正背图检查。

本地 `制作留档/03_处理与验证/05_完整院落环境/` 保存1920×1080全景/近景/高低/轻DOF/合法极限/背面/三装饰/四挂点图、对照网页及验证JSON。6田射线只证明字段碰撞命中，视觉遮挡由截图目检，二者不混淆。合法相机yaw[-12,68]、pitch[28,58]、概览distance[22,34]、平移±2；180°背面为额外结构压力检查。

图中动态水、反射、时段光色和灯笼光留3.4；样片所有田统一放青菜是环境可见性标尺，正式两作物全阶段由3.1/3.3接入。没有扩展航行、建筑或角色系统。没有宣称静帧是实时性能证明；3.5/4.2据正式场景计时选择档位。

接缝复核：田地、土畦和岛顶共用世界坐标草土颜色／法线，修复未提供切线的网格使用NORMAL_MAP导致的块状明暗；渐低土畦必须与田格边缘齐高。建筑只在基脚增加窄幅风化色过渡，桌篮／柱脚的接触暗部由SSAO补充，不能把平面贴花当作真实光照或用草簇遮盖所有接缝。当前实现与实景A/B证据见开发准备。


## 前景节点：20260918 低机位边角框景

用户固定机位（俯角16.5°、偏航27.5°、距离28.6、视野29°、中心0.25/0.75/0）后，原程序前景露出两块完整扁底座，枝条稀疏且叶片单色，像水上盆景。本次替换只在 `Game/presentation/camera_foreground.gd` 组合已有模型，不修改生成原件、不另付费：

- 两株桂花来自 [Trees/20260917/osmanthus](Trees/README.md) 的原高档／连续风动权重，左大右小，枝干与叶簇整体复用，没有切树冠或生成贴片。
- 八组不同尺度的竹、十组矮野花，沿用 [Bamboo](Bamboo/)／[Flowers](Flowers/) 的参考→Tripo→Blender现役高档，保留贴图、网格和植物风动。没有新增绑定或拆件。
- 八块岸石复用 [Rocks/20260917](Rocks/README.md) 的 stone_4/0/3/2，每侧四块不等比例、埋深、方向错开；保留原石材纹理，删除旧前景覆盖材质导致的单色扁块。现役路径均在 `Game/art/environment/`，没有复制一套资源。
- 两组根部按选定镜头的底角延伸到画外，仍为固定世界位置；镜头转动有视差，聚焦、布置、自由检查和低画质退出前景。无新增碰撞体、落叶发射器或光源；阴影关闭，LOD bias为1，导入文件及模型面数未改。总源网格306038三角、28材质表面（旧版14844／16），这不是实际可见绘制量或性能达标数据，本轮未进行性能基准测试。
- `focus_detail.gd` 全景模糊强度0.115，近景清晰边界为最近田块深度减4米、过渡3.5米。首轮影响了船，已收窄近景模糊范围；远景过渡加长至500米以避免共享模糊强度把原绘景过度抹平。聚焦景深算法及用户开关保持原有行为。

### 下沿水草与浮叶补充

上述边角框景为首版（`2ce9876`，306038源三角／28表面）。用户随后指出前景下沿水面过空，第二轮在同一脚本增加两片不对称水草浅岸、八块复用河石，以及两小簇现役P2荷花；中央保留通透的水面通道。相机和景深参数不变，无新增图片、Tripo或Blender任务及费用。

- 水草由Godot生成204片弯曲折叶，合并为六个网格（9384三角），固定随机种子917680／917681；长度、弯曲、朝向与三种草色错开，左高右低。使用现有grass风动，根部固定，无逐叶物理模拟。新增石头使用stone_0/3/1/4，基部延伸到画外。
- 荷花复用[已有P2高档](Lotus/README.md)，视口落点约(0.43,0.90)与(0.64,0.86)，保持世界坐标并做小幅浮动。其原模型根部低于浮叶0.15米，吃水深度必须乘实例缩放，不能直接把根部放在水面，否则茎会悬空。
- 第二轮前景合计340742源三角／44表面，比首版增加34704／16；非实际可见绘制量，本轮没有性能基准测试。仍随聚焦、自由检查和低画质退让。
- 第二轮证据独立保存在`.local/verification/foreground-shore-20260918/`：01全景、02早晨、03关景深、04左右转角、05聚焦、06小窗口、07四千像素、08夜间与report.json。定向原生检查通过并检视画面，未录视频，用户主观效果待确认。按首版提交与本次提交可补拍“下沿空水面→错落水草与浮叶”。复现命令见[开发准备](../../docs/development-setup.md)。

### 首版对比素材与复现

旧源码基线 `246b83f`。同一机位、隔离新农场、16:30光照的前后图位于 `.local/verification/foreground-20260918/before/01-overview.png` 与 `after/01-overview.png`；after另存早晨、关闭景深、左右偏航10°、聚焦、960×600、4K与夜间。HUD时间是系统时钟，不是该测试的光照时间，风动相位也未冻结；可作为构图对照，不作为逐像素单变量测量。

`tests/foreground_composition_test.gd` 是定向复现入口，使用正式模型、独立存档，不抢主屏焦点；验证六田清晰深度、前景双侧可见、景深开关和聚焦／自由视角／低画质退让。原生检查通过且截图已人工检视。用户主观效果待确认，本轮无成片；未来视频可按旧源码和当前源码补拍同机位，资产参考和原模仍由上述目录提供，不重新生成。

## 主岛院落与水禽（20260918）

14种新模型、955积分、语义分件／禽类绑定／风动与水禽、环绕雾及视频节点，统一入口：[CourtyardLife](CourtyardLife/README.md)。

### 2026-09-21 镜头与拍照回归

删除前景投影进入田块保护矩形就直接整组隐藏的逻辑；普通镜头与拍照使用真实深度遮挡和视锥裁切，聚焦／布置仍平滑退让，调试自由检查与低画质仍隐藏。相册显式区分拍照和自由检查，取景与保存保留前景和当前景深。远景舞台固定到全景调节基准，不跟随实时偏航。

复核入口 `tests/foreground_composition_test.gd`：覆盖左右边界、上下俯仰、实际快门保存及下半画面像素对照、景深偏好、聚焦退让、960×600／4K和昼夜。Godot 4.7.2 Forward+ 实景通过，证据 `.local/verification/foreground-camera-fix/`；未改模型、贴图或存档格式。
