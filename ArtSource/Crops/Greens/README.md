...
用户否定上一轮鲜绿、缺乏工笔笔触的外观，要求继续用 P2 重试。本轮已替换开发游戏成熟青菜，等待用户主观评价；幼芽、幼株保持原版本。
...
- 原始6776面中5776四边面（85.2%），三角化12552；原模、高低整理网格均0开边／非流形边、0零面积面、0孤点。高档完整保留 **12552三角**，无减面；辅助低档 **8158三角**，四向检查轮廓。宽.48／高.4026／深.4706米，根部原点，原生4K色图、单材质、roughness .97／metallic 0／specular .08。源和重导入审计见 [asset-audit.json](p2-gongbi-20260918/asset-audit.json)。
- 当前农田始终实例化完整高档，导入自动LOD关闭；低档供明确选择与对照，**当前农田没有自动切换到该辅助低档**。不再用测试中的旧3600三角上限约束这颗青菜；定向资产测试核对实际导出审计及高档原模完整性。
- 运行材质只对成熟青菜启用 `preserve_painted_color`，绕过旧版增绿、压暗与降饱和，保留真实光照／接地染土；其他植物与青菜幼年阶段不变。复用根部约束的包围盒微风，无顶点色逐叶权重、语义分割或绑定。
