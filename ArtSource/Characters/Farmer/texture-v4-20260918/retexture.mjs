...
const directory = 'ArtSource/Characters/Farmer/texture-v4-20260918';
...
const settings = {
  ...
  pbr: false, bake: false, texture_alignment: 'geometry', texture_seed: 202609186,
  texture_prompt: {image: {file_token: style.value}}
};
await mkdir(directory, {recursive:true});
await writeFile(directory + '/request.json', JSON.stringify({...settings, texture_prompt:{image:{path:'../tpose-v2-20260918/front.png'}}, input:'ArtSource/Characters/Farmer/tpose-v2-20260918/farmer_gongbi.glb'},null,2));
const result = await runTask({client,request:{endpoint:'/v3/models/texture',payload:{input:mesh.value,...settings}},taskType:'texture',name:'farmer-gongbi-texture-v4',outDir:'.local/farmer-gongbi-texture-v4'});
await writeFile('.local/farmer-gongbi-texture-v4/result.json', JSON.stringify(result,null,2));
