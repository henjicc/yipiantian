"""Restore retextured geometry scale, verify shape, export and reimport GLB.
...
bpy.ops.import_scene.gltf(filepath=str(ROOT.parent / 'tpose-v2-20260918/farmer_gongbi.glb'))
baseline_objects = [o for o in bpy.context.scene.objects if o.type == 'MESH']
baseline_points = [o.matrix_world @ v.co for o in baseline_objects for v in o.data.vertices]
baseline_tree = KDTree(len(baseline_points))
for index, point in enumerate(baseline_points):
    baseline_tree.insert(point, index)
baseline_tree.balance()
baseline_triangles = sum(audit(o)['triangles'] for o in baseline_objects)
bpy.ops.wm.read_factory_settings(use_empty=True)
...
report['max_vertex_distance_m'] = max(baseline_tree.find(o.matrix_world @ v.co)[2] for o in meshes for v in o.data.vertices)
assert report['max_vertex_distance_m'] < 1e-5, 'Retexture changed shape'
assert sum(o['triangles'] for o in report['prepared']) == baseline_triangles, 'Retexture changed triangle count'
