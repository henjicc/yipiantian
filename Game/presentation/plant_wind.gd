...
const PROFILES := {
	...
	"scallion": Vector4(0.0065, 0.28, 0.0, 0.0008),
	"garlic": Vector4(0.0055, 0.30, 0.0, 0.0014),
...
func _apply_node(node: Node, kind: String, preserve_painted_color: bool) -> void:
	if node is MeshInstance3D and node.mesh != null:
		...
		if kind in ["greens", "radish", "spinach", "chrysanthemum", "celery", "tatsoi", "carrot", "scallion", "garlic", "autumn_crop"]:
		...
		mesh.set_instance_shader_parameter("wind_rate", 0.65 if kind in ["greens", "tatsoi"] else (0.82 if kind == "spinach" else 1.0))
