...
static func pose(node: Node3D, entry: Dictionary, plan: RefCounted) -> void:
	var at: Vector3=plan.slots[entry.slot_id] if not entry.slot_id.is_empty() else Vector3(entry.position[0],plan.ground_height_at(Vector2(entry.position[0],entry.position[1]))+.005,entry.position[1])
	...
	elif entry.slot_id in ["hanging_01","hanging_02"]: yaw+=deg_to_rad(plan.angles.house)
	elif entry.slot_id=="hanging_04": yaw+=deg_to_rad(plan.angles.kitchen)
