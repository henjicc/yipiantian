...
func _sync_paths() -> void:
	if _path_source==environment.plan.paths: return
	_path_source=environment.plan.paths.duplicate(true);_paths=IslandSpace.new()
	_entry_checks.clear()
	for route_index: int in environment.plan.paths.size():
		var route: PackedVector3Array=environment.plan.paths[route_index]
...
func entry_issue(entry: Dictionary, plan: RefCounted, check_animals: bool=true) -> String:
	...
	if check_animals:
		...
		for bird: Dictionary in environment.get_node("CourtyardAnimals").birds:
			# Plant feet are rotated rectangles; twice the clearance also encloses
			# their offset corners. Only nearby birds need the exact polygon check.
			if not bounds.grow((bird.radius+.15)*2).has_point(bird.position): continue
...
func _static_entry_issue(entry: Dictionary, plan: RefCounted) -> String:
	...
	if not _paths.collisions(polygon).is_empty(): return "请为道路留出空间。"
	return ""

func update(plan: RefCounted) -> void:
	message=""
	if _shown!=plan.plants or not is_instance_valid(_shore):
		display.update(plan.plants);environment.fit_player_dressing(plan)
	...
	var occupied: Dictionary=Plants.index_entries(plan.plants)
	var present: Dictionary={}
	for entry: Dictionary in plan.plants: present[entry.id]=true
	for id: int in _entry_checks.keys():
