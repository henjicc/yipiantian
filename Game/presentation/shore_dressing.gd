...
static func stones(plan: RefCounted) -> Array[Dictionary]:
	...
	for i: int in base.rim.size():
		...
		for j: int in count:
			...
			if plan.construction.land.is_empty() or not Geometry2D.is_point_in_polygon(Vector2(entry.at.x,entry.at.z),plateau): result.append(entry)
	for i: int in base.shelves.size():
		var point: Vector3=base.shelves[i]
		if not plan.construction.land.is_empty() and Geometry2D.is_point_in_polygon(Vector2(point.x,point.z),plateau): continue
...
static func plants(plan: RefCounted) -> Node3D:
	...
	for p: Vector2 in added_points(plan):
		...
		# Surface profile expands radially at the waterline, as the bank builder does.
		var outward: Vector2=p.normalized()
		var at: Vector2=p*1.06+outward*.25
