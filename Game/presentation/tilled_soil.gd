...
static func height_at(p: Vector2, half_extent: Vector2 = Vector2(1.2,.88), row_span: float = .44) -> float:
	...
	var furrow: float = sin((p.y + half_extent.y) * TAU / row_span + sin(p.x * 5.0) * .16)
...
static func patch(center: Vector3, span: Vector2, field_seed: int = 0, half_extent: Vector2 = Vector2(1.2,.88)) -> ArrayMesh:
	...
	for row: int in 16:
		for col: int in 20:
			for offset: Vector2i in [Vector2i(0,0),Vector2i(1,0),Vector2i(1,1),Vector2i(0,0),Vector2i(1,1),Vector2i(0,1)]:
				...
				var e: float = .002
				var dx: float = (height_at(p + Vector2(e,0),half_extent,span.y) - height_at(p - Vector2(e,0),half_extent,span.y)) / (2.0*e)
				var dz: float = (height_at(p + Vector2(0,e),half_extent,span.y) - height_at(p - Vector2(0,e),half_extent,span.y)) / (2.0*e)
				surface.set_uv(uv)
				surface.set_color(Color(0,0,0,1))
				surface.set_normal(Vector3(-dx, 1.0, -dz).normalized())
				surface.add_vertex(Vector3(local.x, height_at(p,half_extent,span.y), local.y))
	...
	for index: int in grain_count+28:
		...
		if index < grain_count/2:
			p = (clusters[index%5] + Vector2(rng.randf_range(-.05,.05),rng.randf_range(-.04,.04))).clamp(-spread,spread)
		if contact:
		...
		var at := Vector3(p.x, height_at(p + Vector2(center.x,center.z),half_extent,span.y) - size*rng.randf_range(.15,.40), p.y)
