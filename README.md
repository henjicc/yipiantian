...
Windows 普通窗口版三维微缩农场。六块大田各分为4×4小格，同一块田可混种青菜和白萝卜，按格播种、浇水、现实时间生长与收获；江南院落含三件可解锁装饰、昼夜、声音、焦点细节及景深。农场和声音／显示设置分别保存在当前用户目录。桌面壁纸宿主、账号联网和云存档不在本版本范围。
...
操作：左键点击大田靠近，待镜头到达后点击小格，再选择作物并点击播种、浇水或收获按钮；按钮立即作用于所选格，世界点击只选择，不自动种植。滚轮调整距离，中键微调角度，Shift + 中键平移。“全景”返回，“复位”恢复构图。Esc / 右键先关闭当前弹窗／取消装饰预览；在农田内先取消所选格，再返回全景。“布置”处理已解锁装饰；“设置”包含三路音量、窗口／全屏、画质、景深开关、操作与来源。新操作随本轮源码推进；历史候选包的随包说明仅对应其自身版本。
...
上述 rc.1／rc.2 的截图、录屏、性能和独立包验收均不覆盖当前逐格玩法与画面重构；新候选提交与验收尚未冻结，不提前宣布 rc.3 通过。
...
- 游戏存档使用 `%APPDATA%/Godot/app_userdata/我有一片田/farm/`，偏好使用同级 `preferences/`。主农场为v3 `farm.json`，上一有效副本为 `farm.backup.json`。读取v1／v2时，每旧田唯一作物迁入第2行第2列（`cell_06`），其余格为空，保留生长、浇水、时间、累计和已有装饰；首次升级写入前保留按旧版号与SHA256命名的不可变原件，不能拿旧版程序覆盖v3。坏件、未来版本与写入失败不会被静默重置。恢复入口和安全备份见随包说明。测试同时注入隔离 farm 与 settings；普通发行验证使用 `tests/start-isolated-game.ps1` 的进程级 APPDATA / LOCALAPPDATA，不能覆盖玩家数据。
