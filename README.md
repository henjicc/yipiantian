...
当前分享版本为 **Demo 0.1.4（开发中试玩版）**：[Windows 可运行文件夹](<.local/releases/0.1.4-23250882/我有一片田 Demo 0.1.4 Windows/>)，源码 `23250882`。双击 `Farm.exe`，无需解压。修正0.1.3的错误方案：壁纸游玩和观赏始终在桌面图标下方，保持系统鼠标、图标和任务栏原样；输入采用图标矩形排除，转发鼠标点击、滚轮、拖动和修饰键。保留任务栏避让、游玩不限壁纸帧率、选工具后靠近农田保留工具，以及座椅休息／继续保留视角与页面。按此前要求仅构建，不运行游戏验收。构建入口 `scripts/package-release.ps1 -Commit 23250882 -Version 0.1.4 -FolderOnly -ReuseImportCache`，证据在 `.local/releases/0.1.4-23250882/evidence/`。
