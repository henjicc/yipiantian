...
当前分享版本为 **Demo 0.1.3（开发中试玩版）**：[Windows 可运行文件夹](<.local/releases/0.1.3-73699975/我有一片田 Demo 0.1.3 Windows/>)，源码 `73699975`。双击 `Farm.exe`，无需解压。桌面游玩使用原生鼠标、滚轮和拖动，暂时遮挡本屏桌面图标，保持任务栏可用；游玩取消壁纸帧率上限，底部菜单避让任务栏。选工具后靠近农田保留工具，座椅休息／继续保留当前视角和页面。保留十次关于解锁开发者模式和全部作物成熟。按用户要求只构建，不运行测试或开发预览。构建入口 `scripts/package-release.ps1 -Commit 73699975 -Version 0.1.3 -FolderOnly -ReuseImportCache`，固定源码独立克隆并复用同引擎导入缓存；证据在 `.local/releases/0.1.3-73699975/evidence/`。
