...
- 当前开发存档使用 `%APPDATA%/Godot/app_userdata/我有一片田/farm-v5/`，偏好使用同级 `preferences/`。主农场为 v5 `farm.json`，包含田块布局与作物，上一有效副本为 `farm.backup.json`。早期开发不维护旧版兼容：旧 `farm/`、`farm-v4/` 和留存版保持原件，不迁移历史数据。坏件、未知版本与写入失败不会被静默重置。测试同时注入隔离 farm 与 settings；普通发行验证使用 `tests/start-isolated-game.ps1` 的进程级 APPDATA / LOCALAPPDATA，不能覆盖玩家数据。
