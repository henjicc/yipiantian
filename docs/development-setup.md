...
2026-09-18，Godot 4.7.2：此前项目1280×720逻辑画布与部分场景截图强设1920×1080并存，导致同分辨率下UI比例不一致；自定义硬件指针又固定48物理像素，在4K显得过小。现项目统一1600×900、canvas_items缩放；录制保持4K渲染，不单独改变UI逻辑画布，视觉检查移除独立content_scale_size覆盖。硬件指针按64逻辑像素乘实际stretch比例，4K约154像素，上限256；携带图标继续使用逻辑坐标。小窗点击测试使用 `root.push_input(event, true)`，防止逻辑坐标重复换算。
