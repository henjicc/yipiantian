...
| Tripo | 插件 0.2.2 + CLI 0.4.0；复用已有国际区登录，生成质量待真实资产验证 |
...
- 本轮仅配置与比较，未提交生成、上传参考文件或消耗生成积分；尚无 Tripo 资产可声称通过 Blender / Godot 验收。

各类资产使用 Godot、Blender、Tripo 的分工、Tripo 内部模型选择及交接验收，统一见 [三维资产工具分工](asset-workflow.md)。Tripo 首次生成验证优先选一个复杂而独立的静态道具；简单道具、拼接件与作物阶段不因工具已安装就默认走生成流程。生成服务仅用于开发，不接进游戏运行逻辑。
