...
2026-09-17 的 [1.2 实机样片交接](../task/首个可发布版本/handoffs/1.2-handoff.md) 已将成熟青菜与民居转入同一 Godot 4.7.2 场景，完成高低细节、正背、微调和夜景校对。线性色调映射、环境光 .40、主光 .80／`fff8ea` 比最初偏暖的 FILMIC 设置更能保留灰瓦与叶色；这组参数供正式昼夜任务起步，不能替代最终场景对照。温和景深样片让整田保持清晰、前后层次柔化，具体范围随焦点控制继续验证。固定 v1 图仍是风格依据，样片中的方岛、球树和体块桥船不是最终环境。
