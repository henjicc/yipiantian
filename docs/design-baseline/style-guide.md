...
2026-09-17 的 [1.2 实机样片交接](../task/首个可发布版本/handoffs/1.2-handoff.md) 已将成熟青菜与民居转入同一 Godot 4.7.2 场景，完成高低细节、正背、微调和夜景校对。线性色调映射、环境光 .40、主光 .80／`fff8ea` 曾是 1.2 样片的起点；2026-09-17 与固定 v1 图逐项量化对照后已被推翻，现行为 AgX 色调映射、日间环境光大幅下调、天空光随时钟变暖，依据与数据见[开发准备](../development-setup.md)的画面基线一节。温和景深样片让整田保持清晰、前后层次柔化，具体范围随焦点控制继续验证。固定 v1 图仍是风格依据，样片中的方岛、球树和体块桥船不是最终环境。
