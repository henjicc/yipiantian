...
2026-09-18 [P2青菜](../ArtSource/Crops/Greens/README.md)首轮外观被用户否定：参考偏离工笔基准，原生成有异常薄片，本地又为旧测试上限从9286减至3399三角，运行时叠加增绿。后续以固定工笔基准重新出单体图，P2 + v3.5 detailed、`delight=false`、original_image，新增120积分；原生四边面占85.2%，原模与高档完整保留12552三角、4K色图，0开边。成熟青菜绕过旧调色、沿用根部微风，实景可见水墨色斑与细线；用户审美待签收。多项输入同时变化，不能将收益单归于delight或面数。
...
3. 青菜成熟株已用P2重制工笔小样，先等待成熟株风格评价，再决定幼株是否跟进。多视图仍可用于背面结构存疑的作物；不能把首轮空间位置相位估计称为真实逐叶分件。
