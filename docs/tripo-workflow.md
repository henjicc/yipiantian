...
2026-09-17石头实测：五张独立图片工具参考分别生成H3.1 Ultra＋v3.5 HD，五次各60积分；单体低模合计6200三角形，在岸边、田边与路面复用。显式传入H3.1型号，避免CLI按低面数预算自动改走其他模型。原始参数、图→模型对应关系和后处理边界见[石头记录](../ArtSource/Environment/Rocks/README.md)。后续任何Tripo生成都先按资产工作流准备图片工具参考，本节的路线对比也不例外。
