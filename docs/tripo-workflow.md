...
2026-09-18 增加 [P2 青菜成熟株](../ArtSource/Crops/Greens/README.md)：P2 四边面 + v3.5 detailed + delight，120 积分；原始 5617 面中 3669 四边面（约 65%），运行档 3399/1399 三角形。P2 源被程序判定为正立（按纹理绿度区分根与冠），顶点色承载风动权重（弯曲/叶片抖动/逐叶相位），游戏按颜色层存在与否自动选择 authored 风动，弯曲幅度按株高另行设定。画面采用待用户签收；P2 在四边面占比和非流形边上仍需逐件审计，不能宣称开箱即用。
...
| 青菜成熟株，带风动权重 | 秋季批单体参考图；`P2-20260801`、四边面、4500面请求；纹理 v3.5、detailed、delight、无 PBR；种子 202609191 | 程序按纹理绿度判定正立方向，整理出 3399／1399 三角两档并写入顶点色风动权重（r 弯曲、g 叶片抖动、b 逐叶相位）；游戏按颜色层存在自动走 authored 风动，幼芽／幼株保持包围盒风动。两档 `generate_lods=false`。见[青菜源](../ArtSource/Crops/Greens/README.md) |
...
3. ~~青菜比较多视图 H3.1 与 P2~~ 青菜成熟株已于 2026-09-18 实际采用 P2（单视图、顶点色风动权重，见上表）；幼株是否跟进 P2 待成熟株签收后决定，多视图比较仍可用于背面结构存疑的作物。
