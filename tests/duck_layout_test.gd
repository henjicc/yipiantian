...
func ready_draft() -> bool:
	...
	while is_instance_valid(scene.island_builder.duck_preview) and scene.island_builder.duck_preview.pending:
		await process_frame
...
func _run() -> void:
	...
	var models: Dictionary=builder.duck_preview.models.duplicate()
	var start: Vector2=scene.camera.unproject_position(Vector3(-12,.13,4))
	await mouse(start,true)
	for point: Vector3 in [Vector3(-8,.13,9),Vector3(-8,.13,9.5),Vector3(-8,.13,10)]:
	...
	expect(is_instance_valid(builder.duck_preview) and builder.duck_preview.models.LakeDuck1==models.LakeDuck1,"Continuous area dragging reuses displayed duck models")
	expect(scene.farm_state.snapshot()==snapshot,"Held water-region draft does not publish state")
	await mouse(scene.camera.unproject_position(Vector3(-8,.13,10)),false)
	if not await ready_draft(): await finish();return
	await shot("01-six-preview")
	var preview_node: Node3D=builder.duck_preview.models.LakeDuck6
	...
	var region: RefCounted=animals.duck_space
	...
	expect(builder._status.text.contains("未能保存") and is_instance_valid(builder.duck_preview),"Failed save keeps retryable preview")
	scene.store.directory=usual
	if not await apply(): await finish();return
	expect(animals.duck_space==region,"Count-only save reuses the baked region")
	...
	await drag(Vector3(-12,.13,4),Vector3(-8,.13,9.5))
	builder.cancel_draft()
	await drag(Vector3(-12,.13,4),Vector3(-8,.13,9))
	if not await ready_draft(): await finish();return
	expect(builder.duck_preview.validated.construction.ducks.area==builder.draft.construction.ducks.area,"Late cancelled water check cannot overwrite new draft")
	builder.cancel_draft();await frames()
	expect(not animals.duck_preview and original_nodes.LakeDuck1.visible,"Cancel restores original animals and motion")
	...
	expect(animals.duck_space.resting.size()==4,"Every offered rest preference has a valid point")
...
func edges(animals: Node3D) -> void:
	...
	await mouse(scene.camera.unproject_position(Vector3(-12,.13,4)),true)
	var event:=InputEventMouseMotion.new();event.position=scene.camera.unproject_position(Vector3(-8,.13,10));event.button_mask=MOUSE_BUTTON_MASK_LEFT
	...
	builder.cancel_draft();await frames()
	expect(not animals.duck_preview and first.node.visible,"Cancelled preview restores visibility and motion")
	builder._values.count.value=6
	await drag(Vector3(-2,.13,0),Vector3(2,.13,5))
	expect(not builder.issue().is_empty() and builder._confirm.disabled,"Land cannot become a waterfowl region")
	expect(scene.farm_state.snapshot().layout.construction.ducks.count==12,"Invalid region does not change the live flock")
	...
	builder._values.count.value=7
	expect(builder.duck_preview.pending and not animals.water_ready,"Flock waits for actual terrain water revision")
	...
	expect(animals.water!=old_water and animals.yard!=yard and animals.interaction.find("LakeDuck7").space==animals.duck_space,"Flock uses refreshed water, not stale worker output")
	for bird: Dictionary in animals.birds:
		if bird.kind=="duck": expect(bird.space==animals.duck_space and bird.space.contains(bird.position),"All ducks use committed current region")
