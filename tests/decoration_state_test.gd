...
func _run() -> void:
	...
	_expect(lived.snapshot().bench.unlocked and not lived.snapshot().drying_rack.unlocked and lived.snapshot().tea_table.unlocked,"Resting furniture is free; additional production is earned")
	lived.unlock({}, {"records":{"leaf_stir":{"shared":1}}})
